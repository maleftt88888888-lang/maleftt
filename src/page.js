...
.topbar { position:sticky; top:0; z-index:1200; display:flex; align-items:center; justify-content:space-between; padding:12px 16px; background:rgba(10,12,17,.85); -webkit-backdrop-filter:blur(14px); backdrop-filter:blur(14px); border-bottom:1px solid var(--line); font-size:14px; color:var(--txt); font-weight:700; }
.topbar .title { display:flex; align-items:center; gap:8px; }
.topbar .title::before { content:""; width:4px; height:14px; border-radius:2px; background:linear-gradient(180deg,var(--cyan),var(--green)); }

/* ---- map + controls ---- */
#map { height:48vh; width:100%; min-height:260px; background:#0a0c11; border-bottom:1px solid var(--line); }
...
<body>
<div class="topbar">
  <span class="title">iOS Location Spoofer</span>
</div>
...
<div class="panel">
  ...
  <div class="card">
    ...
    <div class="input-row">
      <input id="searchInput" data-i18n-ph="search_ph" placeholder="搜索国内/全球地名、POI" />
...
function transformLng(x, y) {
  var ret = 300.0 + x + 2.0 * y + 0.1 * x * x + 0.1 * x * y + 0.1 * Math.sqrt(已为你进行了深度优化与清理。本次优化完成了以下重点改动：

### 🛠️ 优化与改进说明：
1. **彻底去除所有广告与水印**：
   - 删除了顶部 YouTube 频道链接、TG 群推广和作者 Fork 声明。
   - 删除了红色的防售卖警示框（`.redbox`）和 YouTube 视频教程按钮（`.vidbtn`）。
   - **完全抹除平铺水印系统**：删除了 `.wm` 样式、`buildWM()`、`ensureWM()` 监听器及定时巡检逻辑，保持页面纯净简洁。
2. **国内搜索与高德地图高精度纠偏优化**：
   - **内置 GCJ-02（火星坐标系）与 WGS-84 自动转换算法**：国内高德/腾讯等地图使用的是 GCJ-02 坐标系，而 iOS 系统改机定位需要真实的 WGS-84 坐标。本优化会自动识别国内坐标并精准纠偏，消除数百米的定位偏差。
   - **国内搜索增强**：`searchPlace()` 优先尝试后台 `/api/search` 接口（配合高德 Web API），若未配置则自动降级使用 OSM Nominatim 中文搜索，并对搜索出来的国内地点自动进行 WGS-84 坐标转换。
   - **支持识别粘贴国内各大地图坐标**（高德、百度、腾讯、Apple 地图文本）。

---

由于完整代码较长，按要求为你**分两段发送**。

### 📄 第一段代码（HTML 结构 + 暗黑 CSS 样式 + 双语字典）
